...
spark.sql('create database if not exists firasf_catalog.dbt;')
incrementally_ingest_folder('/dbdemos/dbt-retail/users', 'json', 'firasf_catalog.dbt.dbt_c360_bronze_users')
incrementally_ingest_folder('/dbdemos/dbt-retail/orders', 'json', 'firasf_catalog.dbt.dbt_c360_bronze_orders')
incrementally_ingest_folder('/dbdemos/dbt-retail/events', 'csv', 'firasf_catalog.dbt.dbt_c360_bronze_events')

print('Congrats, our new data has been consumed and incrementally added to our bronze tables')

# COMMAND ----------

# MAGIC %sql SELECT * FROM firasf_catalog.dbt.dbt_c360_bronze_users
